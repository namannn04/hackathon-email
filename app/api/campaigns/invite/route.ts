...
import { createCampaignInvite, revokeCampaignInvite } from '@/lib/invites/access';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const invite = await createCampaignInvite(
      readString(body.campaignId, 'Event', 80),
      actor,
      origin,
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const invite = await revokeCampaignInvite(
      readString(body.inviteId, 'Invitation', 80),
      actor,
    );
    return NextResponse.json({ revoked: true, campaignId: invite.campaignId });
  } catch (error) {
    return jsonError(error);
  }
}
