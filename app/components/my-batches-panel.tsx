  ...
  return (
    <>
      ...
      <section className="mb-6 rounded-[20px] border border-[#dcdcd5] bg-white p-5 sm:p-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h2 className="text-[15px] font-semibold">Connected Gmail accounts</h2><p className="mt-1 text-xs text-[#77776f]">Every account is checked for Gmail send permission before it appears in the sender list.</p></div>{overview.mockTransport ? <button onClick={() => void onAddMockAccount()} className="h-10 rounded-xl bg-[#263d32] px-4 text-sm font-semibold text-white">Add test Gmail</button> : <a href="/api/gmail/connect?returnTo=%2Fmy-batches" className="inline-flex h-10 items-center rounded-xl bg-[#263d32] px-4 text-sm font-semibold text-white">Connect another Gmail</a>}</div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{overview.gmailAccounts.length ? overview.gmailAccounts.map((account) => <div key={account.id} className={`rounded-2xl border p-4 ${account.canSend ? 'border-[#d7e3da] bg-[#f8fbf9]' : 'border-[#ecd9bd] bg-[#fffaf2]'}`}><div className="flex items-start justify-between gap-3"><p className="min-w-0 truncate text-sm font-semibold">{account.email}</p><span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-bold uppercase tracking-[.08em] ${account.canSend ? 'bg-[#e3f1e7] text-[#34704c]' : 'bg-[#f7e8cd] text-[#8a5b16]'}`}>{account.canSend ? 'Ready' : 'Reconnect'}</span></div><p className="mt-1 text-xs text-[#77776f]">{account.displayName ?? 'Google account'}</p><p className={`mt-3 text-xs leading-5 ${account.canSend ? 'text-[#53705e]' : 'text-[#855d22]'}`}>{account.message}</p><div className="mt-3 flex items-center gap-4">{!account.canSend && !overview.mockTransport ? <a href={`/api/gmail/connect?returnTo=%2Fmy-batches&accountId=${encodeURIComponent(account.id)}`} className="text-xs font-semibold text-[#76500f] underline underline-offset-4">Reconnect</a> : null}<button type="button" onClick={() => { if (window.confirm(`Disconnect ${account.email}? Relay will no longer be able to send from it.`)) void onDisconnectAccount(account.id); }} className="text-xs font-medium text-[#98523b] underline underline-offset-4">Disconnect</button></div></div>) : <p className="text-sm text-[#77776f]">No Gmail account connected yet.</p>}</div>
      </section>
