...
export function AdminPanel({ overview, onCreateEvent, onCreateMailTask, onAddSuppression, onRemoveSuppression, onCreateInvite, onRevokeInvite }: {
  ...
  onCreateMailTask: (input: { eventId: string; name: string; toEmail: string; subject: string; bodyText: string; bodyHtml?: string; batchSize: number }) => Promise<unknown>;
  ...
  async function submitTask(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await onCreateMailTask({ eventId: event.id, name: String(form.get('name')), toEmail: String(form.get('toEmail')), subject: String(form.get('subject')), bodyText: String(form.get('bodyText')), bodyHtml: String(form.get('bodyHtml') ?? '').trim() || undefined, batchSize: Number(form.get('batchSize')) });
