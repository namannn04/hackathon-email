...
export function AdminPanel({ overview, onCreateEvent, onCreateMailTask, onAddSuppression, onRemoveSuppression, onCreateInvite, onRevokeInvite }: {
  ...
  onCreateMailTask: (input: { eventId: string; name: string; toEmail: string; subject: string; bodyText: string; batchSize: number }) => Promise<unknown>;
  ...
  async function submitTask(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await onCreateMailTask({ eventId: event.id, name: String(form.get('name')), toEmail: String(form.get('toEmail')), subject: String(form.get('subject')), bodyText: String(form.get('bodyText')), batchSize: Number(form.get('batchSize')) });
  ...
  return (
    <>
      ...
      <div className="grid items-start gap-6 xl:grid-cols-2">
        ...
        <section className="rounded-[20px] border border-[#dcdcd5] bg-white p-5 sm:p-6"><h2 className="text-[15px] font-semibold">Create mail task {event ? `for ${event.name}` : ''}</h2><p className="mt-1 text-xs leading-5 text-[#77776f]">Each set becomes one message. Remainders under 100 merge into the previous set when Gmail’s 500-recipient limit allows.</p><form onSubmit={submitTask} className="mt-5 space-y-4"><Field label="Mail task name"><input name="name" required maxLength={120} disabled={!event} className="field-input" placeholder="Reminder — 3 days before" /></Field><Field label="Fixed To address"><input name="toEmail" required type="email" disabled={!event} className="field-input" placeholder="organizer@example.com" /></Field><Field label="Subject"><input name="subject" required maxLength={180} disabled={!event} className="field-input" /></Field><Field label="Body"><textarea name="bodyText" required rows={7} maxLength={50000} disabled={!event} className="field-input min-h-36 resize-y py-3" /></Field><Field label="Target set size"><input name="batchSize" required type="number" min={1} max={499} defaultValue={300} disabled={!event} className="field-input" /></Field><button disabled={!event || taskWorking} className="h-11 rounded-xl bg-[#263d32] px-5 text-sm font-semibold text-white disabled:opacity-50">{taskWorking ? 'Creating sets…' : 'Create mail task and sets'}</button></form></section>
      </div>
