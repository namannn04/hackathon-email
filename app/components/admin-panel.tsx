...
export function AdminPanel({
  ...
  if (overview.user.role !== 'ORGANIZER') {
    return <div className="rounded-2xl border border-[#deded8] bg-white p-8"><h1 className="text-xl font-semibold">Organizer access required</h1><p className="mt-2 text-sm text-[#77776f]">Your account does not have permission to manage campaigns.</p></div>;
  }
  ...
  return (
    <>
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-semibold tracking-[-0.03em] sm:text-[30px]">Campaign control room</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[#686861]">Create a campaign once. Relay validates, deduplicates, suppresses, and batches the entire file automatically.</p>
      </div>

      <section className="mb-6 grid gap-3 sm:grid-cols-3">
        <AdminMetric label="Campaigns" value={String(overview.campaigns.length)} note={`${overview.campaigns.filter((campaign) => campaign.status === 'ACTIVE').length} active`} />
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(340px,.8fr)]">
        <section className="rounded-[20px] border border-[#dcdcd5] bg-white p-5 sm:p-7">
          <div className="mb-6">
            <h2 className="text-lg font-semibold tracking-[-0.02em]">Create campaign</h2>
            <p className="mt-1 text-sm text-[#77776f]">CSV and XLSX files up to 10 MB · batches capped at 500 recipients</p>
          </div>
          <form onSubmit={submit} className="space-y-5">
            <Field label="Campaign name"><input name="name" required maxLength={120} placeholder="HackNova 2026 outreach" className="field-input" /></Field>
            ...
            <button disabled={submitting} className="inline-flex h-11 items-center justify-center rounded-xl bg-[#263d32] px-5 text-sm font-semibold text-white disabled:opacity-50">{submitting ? 'Validating and batching…' : 'Create campaign'}</button>
          </form>

          {result ? (
            <div className="mt-6 rounded-2xl border border-[#cfe0d5] bg-[#f0f6f2] p-4 text-sm text-[#315e43]">
              <p className="font-semibold">Campaign is ready with {result.batches} batches.</p>
              ...
              <a href={`/?campaignId=${encodeURIComponent(result.campaignId)}`} className="mt-3 inline-flex font-semibold underline underline-offset-4">Open campaign →</a>
            </div>
          ) : null}
        </section>
